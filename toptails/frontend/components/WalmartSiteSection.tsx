...
export default function WalmartSiteSection() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch("/api/products/walmart", { cache: "no-store" });
      const body = (await res.json()) as WalmartProductsResponse & { error?: string };
      if (!res.ok) {
  ...
  return (
    <section className="rounded-xl border border-stone-200/90 bg-white shadow-sm shadow-stone-950/5 overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-stone-100 bg-stone-50/80">
        <div className="flex items-center gap-3 min-w-0">
          <span
            ...
            aria-hidden
          />
          <span className="font-semibold text-stone-900 truncate">Walmart</span>
        </div>
        {isBlocked ? (
      ...
      <div className="p-4 sm:p-5">
        ...
        {loadingProducts ? (
          <p className="text-sm text-stone-500 py-2">Loading Walmart products…</p>
        ) : loadError ? (
          ...
          <p className="text-sm text-stone-500 py-2">
            Walmart products are not loaded automatically. Click{" "}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {products.map((p, i) => (
              <ProductCard
                key={`walmart-${i}-${p.product_url ?? p.title}`}
                {...p}
                source_site="walmart"
                rank={i + 1}
              />
